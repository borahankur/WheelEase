...
const WheelChairBooking = () => {
    ...
    useEffect(() => {
        const intervalId = setInterval(async () => {
            try {
                await axiosInstance.post(`/hospital/${currentHospital?._id}/wheelchair/updateStatus`, { equipment: "wheelchair" })
            } catch (error) {
                console.log("Error: ", error)
            }
        }, 2 * 60 * 1000); //checks for updates every two minutes
    ...
    const handleBooking = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const success = await createBooking(userId, isPm, selectedDay, selectedTime, "wheelchair")
            if (success) {
                navigate(`/hospital/${currentHospital?._id}/wheelchair/booking/confirm`)
            }
        } catch (error) {
    ...
    return (
        <main className="bg-customGreen flex flex-col w-screen min-h-screen overflow-y-hidden">
            ...
            <div className="flex justify-center items-center">
                <img src={`/Wheelchair(self service).png`} alt="self service" className="size-44 px-3 py-3" />
                <div className="px-2 py-4 border border-black bg-customGreen2 rounded-full">
                    Total Wheelchairs Available: <span className="font-bold">{getFloorEquipment(currentfloor)?.wheelchairs}</span>
                </div>
            </div>
            <form onSubmit={handleBooking} className="flex flex-col items-center">
                ...
                <div className="flex gap-2 pt-2 pl-2 justify-evenly">
